...
def main():
    ...
    # Flag voor wijzigingen
    root.unsaved_changes = False

    def on_close():
        if root.unsaved_changes:
            if not messagebox.askyesno(
                "Afsluiten",
                "Er zijn niet-opgeslagen wijzigingen. Weet je zeker dat je wilt afsluiten?"
            ):
                return
        root.destroy()
    ...
    # Nu de statusbalk integreren in mark_dirty/mark_clean
    def mark_dirty(*args, **kwargs):
        root.unsaved_changes = True
        status.config(text="Status: wijzigingen niet opgeslagen")

    def mark_clean():
        root.unsaved_changes = False
        status.config(text="Status: alles opgeslagen")
